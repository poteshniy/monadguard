...
Options
  --name <tool>        tool name as published (required to compute toolId)
...
const tool = { kind: flag('kind', 'mcp'), origin, name: flag('name', origin.split('/').pop()) };
...
try {
  ...
  if (has('json')) console.log(JSON.stringify({ ok: true, ...r }, null, 2));
    ...
    console.log(`CLEAN  ${tool.name}  risk ${r.score}  ${r.scans} scan(s), ${r.attestors.length} attestor(s)`);
  ...
  if (e instanceof MonadGuardBlocked) {
    if (has('json')) console.log(JSON.stringify({ ok: false, reason: e.reason, ...e.result }, null, 2));
      ...
      console.log(`${e.result.verdict.padEnd(6)} ${tool.name}  ${e.reason}`);
      console.log(`       ${e.result.toolId}`);
